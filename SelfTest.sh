...
exec_selftest() {
	...
	server=`grep domain /etc/openvpn/client.ovpn | awk '{print $2}'`
	port=`grep PublicIP /etc/openvpn/client.ovpn | awk '{print $3}'`
	publicIP=`grep PublicIP /etc/openvpn/client.ovpn | awk '{print $2}'`
